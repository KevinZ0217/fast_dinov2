...
from dinov2.models.vision_transformer import PositionalEmbedding
#from dinov2.layers.attention import LoRALinear  # import your class
from torch import compile as torch_compile
from dinov2.layers.attention import LoRAAdapter
...
class SSLMetaArch(nn.Module):
    ...
    def prepare_for_distributed_training(self):
        ...
        # 2) …except the adapters themselves
            for name, param in self.student[k].named_parameters():
                if ".attn.qkv.adapter." in name or "pos_embed" in name:
                    param.requires_grad = True 
            trainable = []
            frozen    = []
            for name, p in self.student[k].named_parameters():
            ...
            self.student[k] = get_fsdp_wrapper(student_model_cfg, modules_to_wrap={LoRAAdapter,BlockChunk,PositionalEmbedding})(self.student[k])
            teacher_model_cfg = self.cfg.compute_precision.teacher[k]
            self.teacher[k] = get_fsdp_wrapper(teacher_model_cfg, modules_to_wrap={LoRAAdapter,BlockChunk,PositionalEmbedding})(self.teacher[k])
