...
from dinov2.layers.attention import LoRALinear
#def get_fsdp_wrapper(model_cfg, modules_to_wrap=set()):
#def get_fsdp_wrapper(model_cfg, modules_to_wrap=set(), ignored_modules=None):
def get_fsdp_wrapper( 
    model_cfg, 
    modules_to_wrap: set = set(), 
    ignored_module_classes: set = None, 
):
    ...
    auto_wrap = ModuleWrapPolicy(modules_to_wrap)

    def wrap(module: nn.Module) -> FSDP:
        # 1) Identify instances to ignore
        ignored: list[nn.Module] = []
        if ignored_module_classes:
            for cls in ignored_module_classes:
                ignored += [m for m in module.modules() if isinstance(m, cls)]
        fsdp_mod=FSDP(
            module,
        ...
        target_dtype = mixed_precision_config.param_dtype
        for m in fsdp_mod.modules():
            if isinstance(m, LoRALinear):
                m.base_weight = m.base_weight.to(target_dtype)
                if m.base_bias is not None:
                    m.base_bias = m.base_bias.to(target_dtype)
        return fsdp_mod
        # 2) Wrap with FSDP in one shot
...
#    fsdp_wrapper = partial(
        ...
        #auto_wrap_policy=ModuleWrapPolicy(modules_to_wrap),
        #ignored_modules=set() if ignored_modules is None else ignored_modules,
    #)
